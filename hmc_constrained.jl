...
println("No. of real solutions: ", num_sol_start_system)
...
function backward_check(x1, v1, x, v, step_size)
  ...
  jj = 0
  # check all solutions, until we find one 
  for j in 1:length(S_p)
    ...
    if norm(x_2 - x) > backward_check_tol
      continue
    else # compute the velocity and check 
      # prepare to compute the Lagrange multiplier lam_v
      ...
      if norm(v_2 - v) < backward_check_tol
